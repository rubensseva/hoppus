...
__USER_TEXT int mem_list_remove(char *ptr) {
    for (int i = 0; i < mem_list_n; i++) {
        if (mem_list[i].base == ptr) {
            return mem_list_remove_i(i);
        }
    }
    printf("MALLOC: Couldnt find pointer to free %x\n", ptr);
    return -1;
}
...
__USER_TEXT void *malloc1(unsigned int size) {
    ...
    uint32_t *heap_start = (uint32_t *)malloc_heap;
    uint32_t *heap_end = (uint32_t *)(malloc_heap + MALLOC_HEAP_SIZE);
    ...
    if (mem_list_n >= MAX_MALLOC_NODES - 1) {
        printf("Max number of malloc nodes reached\n");
        return (void *)NULL;
    }
    unsigned int heap_size = (char *)heap_end - (char *)heap_start;
    if (size > heap_size) {
        printf("MALLOC: Size requested for malloc was to large. Got %d, needs to be under %d\n",
               size, heap_size);
        return (void *)NULL;
    }
    ...
    printf("MALLOC: Couldnt find space for size %d\n", size);
    return (void *)NULL;
}
